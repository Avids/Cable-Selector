...
let suppressCanvasBackdrop = false;
let wireRouting = 'orthogonal';
let activeTool = null;  // ETAP-style tool selection
...
function drawSymbol(ctx, type, x, y, w, h, zoom) {
  ...
  // ETAP-style ANSI/IEC electrical symbols - clean black lines
  ctx.strokeStyle = '#1f2937';
  ctx.fillStyle = '#ffffff';

  switch(type) {
    case 'utility':
      // ANSI utility source symbol - circle with cross
      ctx.beginPath();
      ctx.arc(cx, cy, Math.min(w,h)*0.4, 0, Math.PI*2);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(cx, cy - Math.min(w,h)*0.25);
      ctx.lineTo(cx, cy + Math.min(w,h)*0.25);
      ctx.moveTo(cx - Math.min(w,h)*0.25, cy);
      ctx.lineTo(cx + Math.min(w,h)*0.25, cy);
      ctx.stroke();
      break;

    case 'transformer':
      // ANSI transformer - two circles representing windings
      const coilRadius = Math.min(w,h)*0.3;
      ctx.beginPath();
      ctx.arc(cx, cy - coilRadius*0.8, coilRadius, 0, Math.PI*2);
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(cx, cy + coilRadius*0.8, coilRadius, 0, Math.PI*2);
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.moveTo(cx, y);
      ctx.lineTo(cx, cy - coilRadius*0.2);
      ctx.moveTo(cx, cy + coilRadius*0.2);
      ctx.lineTo(cx, y + h);
      ctx.stroke();
      break;

    case 'panel':
      // Panel/MCC - rectangle with internal dividers
      ctx.strokeRect(x, y, w, h);
      const panelLines = 3;
      for (let i = 1; i < panelLines; i++) {
        const ly = y + (h * i / panelLines);
        ctx.beginPath();
        ctx.moveTo(x + w*0.1, ly);
        ctx.lineTo(x + w*0.9, ly);
        ctx.stroke();
      }
      break;

    case 'breaker':
      // ANSI circuit breaker symbol
      const bw = w * 0.6;
      ctx.beginPath();
      // Top connection
      ctx.moveTo(cx, y);
      ctx.lineTo(cx, y + h*0.25);
      // Breaker body - square with diagonal line
      ctx.rect(cx - bw/2, y + h*0.25, bw, h*0.5);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(cx - bw/2, y + h*0.25);
      ctx.lineTo(cx + bw/2, y + h*0.75);
      ctx.stroke();
      // Bottom connection
      ctx.beginPath();
      ctx.moveTo(cx, y + h*0.75);
      ctx.lineTo(cx, y + h);
      ctx.stroke();
      break;

    case 'fuse':
      // ANSI fuse symbol - rectangle with dashed center line
      const fw = w * 0.5;
      const fh = h * 0.5;
      ctx.beginPath();
      // Top connection
      ...
      ctx.lineTo(cx, y + h*0.25);
      // Fuse body
      ctx.rect(cx - fw/2, y + h*0.25, fw, fh);
      ctx.stroke();
      // Dashed center line
      ctx.setLineDash([3/zoom, 2/zoom]);
      ctx.beginPath();
      ctx.moveTo(cx, y + h*0.25);
      ctx.lineTo(cx, y + h*0.25 + fh);
      ctx.stroke();
      ctx.setLineDash([]);
      // Bottom connection
      ctx.beginPath();
      ctx.moveTo(cx, y + h*0.25 + fh);
      ctx.lineTo(cx, y + h);
      ctx.stroke();
      break;

    case 'bus':
      // Bus bar - thick horizontal line with taps
      ctx.lineWidth = 3/zoom;
      ctx.beginPath();
      ctx.moveTo(x, cy);
      ctx.lineTo(x + w, cy);
      ...
      ctx.lineWidth = lw;
      // Tap connections
      const tapSpacing = w / 4;
      for (let i = 1; i <= 3; i++) {
        ctx.beginPath();
        ctx.moveTo(x + i * tapSpacing, cy);
        ctx.lineTo(x + i * tapSpacing, cy + h*0.3);
        ctx.stroke();
      }
      // Top feed
      ctx.beginPath();
    ...
    case 'cable':
      // Cable/conductor - multiple parallel lines
      ctx.lineWidth = 2/zoom;
      const cableSpacing = w * 0.2;
      // Center conductor (solid)
      ctx.beginPath();
      ctx.moveTo(cx, y);
      ctx.lineTo(cx, y + h);
      ctx.stroke();
      // Outer conductors (dashed)
      ctx.setLineDash([4/zoom, 3/zoom]);
      ctx.beginPath();
      ctx.moveTo(cx - cableSpacing, y);
      ctx.lineTo(cx - cableSpacing, y + h);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(cx + cableSpacing, y);
      ctx.lineTo(cx + cableSpacing, y + h);
    ...
    case 'load':
      // Load symbol - triangle (motor) or circle (general load)
      ctx.beginPath();
      // Top connection
      ctx.moveTo(cx, y);
      ctx.lineTo(cx, y + h*0.2);
      // Triangle for motor load
      const triSize = Math.min(w,h)*0.35;
      ctx.moveTo(cx, y + h*0.2 + triSize);
      ctx.lineTo(cx - triSize, y + h*0.2 + triSize*2);
      ctx.lineTo(cx + triSize, y + h*0.2 + triSize*2);
      ctx.closePath();
      ctx.stroke();
      // Base line
      ctx.beginPath();
      ctx.moveTo(cx - triSize, y + h*0.2 + triSize*2.2);
      ctx.lineTo(cx + triSize, y + h*0.2 + triSize*2.2);
      ctx.stroke();
      break;

    case 'meter':
      // Meter - circle with dial
      ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.lineTo(cx + Math.min(w,h)*0.25, cy - Math.min(w,h)*0.15);
      ctx.stroke();
      // Scale arc
      ctx.beginPath();
      ctx.arc(cx, cy, Math.min(w,h)*0.25, Math.PI*0.8, Math.PI*0.2, true);
      ctx.stroke();
      break;
  }
}
...
// ═══════════════════════════════════════════════════
// DRAG FROM TOOLBOX / ETAP-STYLE TOOL SELECTION
// ═══════════════════════════════════════════════════
...
// ETAP-style: Click tool button to activate, then click canvas to place
function selectTool(type) {
  ...
  // Add active class if a tool is selected
  if (activeTool) {
    const btn = document.getElementById('tool-' + activeTool);
    if (btn) btn.classList.add('active');
    // Switch to select mode when tool is active
    setMode('select');
...
// Place component at world coordinates (for ETAP-style click-to-place)
function placeComponentAt(type, wx, wy) {
  ...
  const node = {
    id: idCounter++,
    type: type,
    x: Math.round((wx - d.w/2) / 10) * 10,
    y: Math.round((wy - d.h/2) / 10) * 10,
    props: Object.assign({}, d.defaults),
  };
  if (node.props.system) {
    node.props.phases = getSystemPhaseCount(node.props.system, node.props.phases);
...
canvas.addEventListener('mousedown', e => {
  ...
  // ETAP-style: If a tool is active, place component on canvas click
  if (activeTool && mode === 'select' && e.button === 0) {
    placeComponentAt(activeTool, x, y);
    // Keep the tool active for multiple placements (like ETAP)
    draw();
    return;
  }
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'Delete' || e.key === 'Backspace') deleteSelected();
  // Escape cancels active tool or deselects
  if (e.key === 'Escape') {
    if (activeTool) {
